...
**The same flow, in a browser.** `python3 scripts/webapp.py` serves a **designed member dashboard** (`scripts/ui.html`) at `localhost:8000` — a greeting, a "How can I help you today?" chat bar with one-tap prompts ("Plan my whole Saturday", "Repaint my door navy?"), a *Today at a glance* strip (guest passes, cabanas, pool/dining status, folio balance), a *Your day, perfectly planned* timeline with Confirmed/Pending states, and *À-la-carte* cards for the six domains. And it is **zero-dependency**: stdlib `http.server` only, no framework, no API key — yet it drives the *same* `ConciergeOrchestrator` as the CLI. What you see is the real propose → gate → confirm/decline → commit flow with the live guardrail audit trail rendered as a table. Beyond the anchor "Plan my Saturday" story, it exposes:
